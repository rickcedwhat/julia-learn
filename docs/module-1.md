...
On this site, chat prompts use **code blocks** with a **copy** icon in the corner. Click it, then paste into Antigravity.
:::
...
```text
The instructor's repo is https://github.com/rickcedwhat/julia-learn.git. It should be added as a Git remote named `upstream` (that's the instructor's project). My own copy on GitHub is usually `origin`. Walk me through: adding `upstream` if it's missing, fetching, and merging `upstream/main` into my current branch. Remind me to commit or stash my work first if that is safer.
