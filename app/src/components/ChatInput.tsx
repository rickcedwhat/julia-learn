...
export default function ChatInput({ onSend, onLibraryOpen, disabled }: Props) {
  ...
  function submit() {
    if (!canSend) return
    onSend(value.trim(), staged.map((s) => s.file))
    setValue('')
    setStaged([])
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const newStaged: StagedImage[] = await Promise.all(
      toAdd.map(
        (file) =>
    ...
    <div className="border-t border-gray-200 bg-white">
          ...
          {staged.map((img, i) => (
            <div key={i} className="relative shrink-0">
              ...
              <button
                ...
                className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-gray-700 text-white rounded-full text-xs flex items-center justify-center leading-none hover:bg-gray-900"
      ...
      <div className="flex items-end gap-2 px-4 py-3">
        ...
        <textarea
          ...
          onKeyDown={(e) => {
            // Enter always inserts a newline — Send button only
        ...
        <button
          ...
          disabled={disabled || staged.length >= MAX_IMAGES}
